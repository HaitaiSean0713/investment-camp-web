...
function toast(msg, error=false){
  const el = $('#toast');
  const icon = error
    ? '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#f43f5e" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" x2="12" y1="8" y2="12"/><line x1="12" x2="12.01" y1="16" y2="16"/></svg>'
    : '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#00f0b5" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="8 12 11 15 16 9"/></svg>';
...
function fatal(msg){
  app.innerHTML = `<div class="auth"><div class="card auth-card"><div class="brand"><span class="brandmark">↗</span><div class="brand-text"><span>Camp Invest</span><small>INVESTMENT CHALLENGE</small></div></div><h1 style="color:#f43f5e">無法載入系統</h1><p>${escapeHtml(msg)}</p><button class="btn primary block" data-action="reload" style="margin-top:16px">重新整理頁面</button></div></div>`;
}
...
function modal(title, html, actions=''){
  ...
  d.innerHTML = `<div class="modal-inner"><div class="modal-head"><h2>${escapeHtml(title)}</h2><button class="btn small ghost" data-action="close-modal" aria-label="關閉視窗" style="font-size:16px;padding:4px 8px">✕</button></div>${html}<div class="form-actions">${actions}</div></div>`;
  d.showModal();
  return d;
}
...
function renderLogin(){
  app.innerHTML = `<div class="auth"><div class="card auth-card"><div class="brand"><span class="brandmark">↗</span><div class="brand-text"><span>Camp Invest</span><small>ADMIN CONSOLE</small></div></div><h1>主持人登入</h1><p>請輸入第一次啟動伺服器時顯示的管理員密碼。</p><form data-form="login"><label>管理員密碼<input type="password" name="password" required autocomplete="current-password" autofocus placeholder="••••••••••••"></label><button class="btn primary block" style="margin-top:8px">登入後台 →</button></form></div></div>`;
}
...
function renderAdmin(){
  ...
  app.innerHTML = `<div class="layout"><aside class="sidebar"><div class="brand"><span class="brandmark">↗</span><div class="brand-text"><span>Camp Invest</span><small>ADMIN CONSOLE</small></div></div><nav class="nav">${adminTabs.map(([id, label]) => `<button data-tab="${id}" class="${state.adminTab===id?'active':''}"><span class="nav-icon">${svgIcons[id]||''}</span><span>${label}</span></button>`).join('')}</nav><div class="sidefoot"><strong style="color:var(--text);display:block;margin-bottom:4px">模擬投資競賽平台</strong>資訊不對稱 · 團隊策略交流</div></aside><main class="main"><div class="topbar"><div><div class="eyebrow">CONTROL ROOM / HOST DASHBOARD</div><h1>${escapeHtml(adminTabs.find(x => x[0] === state.adminTab)?.[1] || '總覽')}</h1></div><div class="top-actions"><select class="select-activity" id="activity-select" aria-label="選擇活動">${state.activities.length ? options(state.activities, state.aid) : '<option value="0">尚無活動</option>'}</select>${btn('＋ 新活動', 'new-activity', 'small')}${btn('登出', 'logout', 'small ghost')}</div></div>${a ? renderAdminTab(a) : renderNoActivity()}</main></div>`;
...
function renderDashboard(a){
  ...
  return `<section class="hero"><div class="hero-num">${a.current_round || '—'}</div><div class="eyebrow">${escapeHtml(a.code)} · ${active ? 'LIVE CHALLENGE' : 'ACTIVITY CONCLUDED'}</div><h1>${escapeHtml(a.name)}</h1><p>目前進度：ROUND ${a.current_round || '—'} &nbsp;·&nbsp; 階段：<strong>${stageName[a.stage] || a.stage}</strong> &nbsp;·&nbsp; ${a.stage==='TRADING' ? '<span class="gain">● 交易開放下單中</span>' : '<span class="muted">○ 交易已鎖定</span>'}</p><div class="hero-actions">${btn('開啟投影畫面 ↗', 'open-presenter', 'primary')} ${btn('複製活動', 'duplicate', 'ghost')}</div></section>
  <section class="section"><div class="grid four"><div class="card metric"><div class="label">CURRENT ROUND</div><div class="value">Round ${a.current_round || '—'}</div><div class="sub">${stageName[a.stage]}</div></div><div class="card metric"><div class="label">ACTIVE TEAMS</div><div class="value">${a.teams.length}</div><div class="sub">已建立隊伍</div></div><div class="card metric"><div class="label">MARKET STOCKS</div><div class="value">${a.stocks.length}</div><div class="sub">檔標的股票</div></div><div class="card metric"><div class="label">PUBLISHED SNAPSHOT</div><div class="value">${a.published_snapshot ? '#' + a.published_snapshot.id : '—'}</div><div class="sub">${a.published_snapshot ? time(a.published_snapshot.published_at) : '尚未公布'}</div></div></div></section>
  <section class="section"><div class="section-head"><h2>現場流程控制（7 步流程引導）</h2>${badge(a.stage==='TRADING'?'● 交易開放中':'○ 交易已鎖定', a.stage==='TRADING'?'green':'red')}</div><div class="card pad quick"><div class="btnrow">${btn('① 發布隊伍情報', 'preview-release', a.stage==='INFORMATION'?'primary':'')}${btn('② 開始交流', 'stage-discussion', a.stage==='DISCUSSION'?'primary':'')}${btn('③ 開放交易', 'stage-trading', a.stage==='TRADING'?'primary':'')}${btn('④ 關閉交易', 'stage-closed', a.stage==='CLOSED'?'primary':'warn')}${btn('⑤ 建立績效快照', 'quick-snapshot', '')}${btn('⑥ 公布績效', 'go-snapshots', a.stage==='RESULT'?'primary':'')}${btn('⑦ 下一回合', 'next-round', '')}</div></div></section>
  <section class="grid two section"><div class="card pad"><div class="section-head"><h2>階段計時器</h2>${badge(a.timer_mode==='AUTOMATIC'?'自動關閉':'手動控制', 'aqua')}</div><div class="value mono" data-clock style="font-size:52px;font-weight:900;letter-spacing:-0.04em;margin:12px 0;color:var(--text)">${formatClock(state.timer)}</div><div class="form-actions" style="margin-top:12px"><input id="timer-minutes" type="number" min="0" value="10" style="width:90px" aria-label="分鐘"><span class="muted tiny" style="align-self:center">分鐘</span>${btn('開始', 'timer-start', 'small primary')}${btn('暫停', 'timer-pause', 'small')}${btn('繼續', 'timer-resume', 'small')}${btn('重設', 'timer-reset', 'small')}${btn('歸零跳過', 'timer-skip', 'small ghost')}</div></div><div class="card pad"><h2>活動概況</h2><p class="muted" style="line-height:1.7">隊伍起始資金 <strong>${dollars(a.initial_cash)}</strong> · 累計成交 <strong>${a.transaction_count}</strong> 筆交易</p><p class="muted" style="line-height:1.7">目前處於 <strong>${stageName[a.stage]}</strong>。切換回合時，系統將自動套用下一回合固定股價並重新計算隊伍市值。</p>${a.status==='FINISHED' ? badge('活動已結束', 'gold') : btn('查看回合與價格表 →', 'go-rounds', 'small') }</div></section>`;
}

function renderSetup(a){
  return `<div class="grid two"><section class="card pad"><h2>活動設定</h2><p class="muted tiny">活動代碼：<strong class="mono" style="color:var(--aqua)">${escapeHtml(a.code)}</strong> · 初始資金：${dollars(a.initial_cash)}</p><form data-form="settings" class="formgrid">${field('活動名稱', 'name', 'text', a.name, 'required')}
  ...
  <label class="check wide"><input type="checkbox" name="performance_report_visible" ${a.performance_report_visible?'checked':''}><span>允許隊伍在結束時查看各自的最終績效報告</span></label><label class="check wide"><input type="checkbox" name="review_visible" ${a.review_visible?'checked':''}><span>開放情報復盤（讓隊伍查看所有已發送情報與接收者）</span></label><div class="wide" style="margin-top:12px"><button class="btn primary">儲存設定</button></div></form></section>
  <section class="card pad"><h2>快速階段切換</h2><p class="muted tiny">直接切換目前回合狀態，下單功能僅在「交易進行中」有效。</p><div class="btnrow" style="margin-top:14px">${['INFORMATION','DISCUSSION','TRADING','CLOSED','RESULT','BREAK'].map(x => btn(stageName[x], `setstage-${x}`, a.stage===x?'primary':'')).join('')}</div><div class="divider"></div><h2>投影大螢幕連結</h2><p class="tiny muted">此畫面為公開唯讀，供教室投影機展示。</p><div class="linkbox" style="margin-top:8px">${escapeHtml(location.origin + '/presenter/' + a.code)}</div><div style="margin-top:12px">${btn('開啟投影畫面 ↗', 'open-presenter', 'small')}</div></section></div><section class="card pad section" style="border-color:rgba(244,63,94,0.3)"><h2>危險操作</h2><p class="muted tiny">刪除「<strong>${escapeHtml(a.name)}</strong>」及其所有隊伍、持股、交易與快照紀錄。此操作不可復原。</p><div style="margin-top:14px">${btn('刪除這個活動', 'delete-activity', 'danger')}</div></section>`;
}

function renderTeams(a){
  return `<div class="grid two"><section class="card pad"><h2>新增參賽隊伍</h2><p class="muted tiny">每隊共用一台手機或平板。建立後將產生專屬加入 QR Code 與連結。</p><form data-form="team" class="formgrid">${field('隊伍名稱', 'name', 'text', '', 'required placeholder="例如：第七小隊" class="wide"')}<div class="wide" style="margin-top:8px"><button class="btn primary">新增隊伍</button></div></form></section><section class="card pad"><h2>隊伍登入機制</h2><p class="muted" style="line-height:1.7">主持人將專屬連結或 QR Code 傳給隊伍。隊伍點擊後即建立登入狀態，無需帳號密碼。</p><div class="notice">注意：重新產生連結會使該隊先前的 QR Code 與已登入裝置失效。</div></section></div><section class="section"><div class="section-head"><h2>隊伍清單</h2>${badge(`${a.teams.length} 隊`, 'aqua')}</div>${a.teams.length ? `<div class="tablewrap"><table><thead><tr><th>隊伍名稱</th><th>建立時間</th><th>專屬加入憑據</th></tr></thead><tbody>${a.teams.map(t => `<tr><td><strong style="font-size:15px">${escapeHtml(t.name)}</strong></td><td>${time(t.created_at)}</td><td>${btn('產生專屬加入 QR / 連結', 'regen-'+t.id, 'small')}</td></tr>`).join('')}</tbody></table></div>` : empty('尚未建立隊伍', '請先建立隊伍，再進行情報分配與交易。')}</section>`;
}
...
function renderTeamEntry(){
  app.innerHTML = `<div class="auth"><div class="card auth-card"><div class="brand"><span class="brandmark">↗</span><div class="brand-text"><span>Camp Invest</span><small>TEAM TERMINAL</small></div></div><h1>隊伍登入入口</h1><p>請掃描主辦方提供的隊伍專屬 QR Code，或點擊隊伍邀請連結登入。</p><div class="notice" style="margin-top:20px">提示：每個加入連結與小隊綁定，請勿轉傳或分享給其他隊伍。</div></div></div>`;
}
...
function renderTeamTab(a){
  switch(state.teamTab){
    ...
    case 'transactions':
    case 'performance':
    case 'review': return '<div class="loading">載入中…</div>';
...
function renderMarket(a){
  return `<section class="hero"><div class="hero-num">${a.round || '—'}</div><div class="eyebrow">${escapeHtml(a.name)} · ROUND ${a.round || '—'}</div><h1>${stageName[a.stage]}</h1><p>${a.stage==='TRADING' ? '本回合固定價格交易進行中，點選股票即可下單。' : '閱讀本隊情報並與其他小隊策略交流，準備下一輪操作。'}</p>${a.timer_running ? `<div class="clock mono" data-clock style="font-size:38px;font-weight:900;color:var(--gold);margin-top:14px">${formatClock(state.timer)}</div>` : ''}</section>
  <section class="section"><div class="grid three"><div class="card metric"><div class="label">可用現金</div><div class="value mono">${dollars(a.portfolio.cash)}</div></div><div class="card metric"><div class="label">持股市值</div><div class="value mono">${dollars(a.portfolio.stock_value)}</div></div><div class="card metric"><div class="label">目前總資產</div><div class="value mono" style="color:var(--aqua)">${dollars(a.portfolio.total_asset)}</div></div></div></section><section class="section"><div class="section-head"><h2>市場股票行情</h2>${badge('本回合價格固定', 'aqua')}</div>${a.stocks.length ? `<div class="stock-grid">${a.stocks.map(s => `<div class="card stock-card" data-stock="${s.id}"><div class="stock-heading"><div style="display:flex;gap:12px;align-items:center"><span class="symbol">${escapeHtml(s.symbol.slice(0,2))}</span><div><div class="stock-name">${escapeHtml(s.name)}</div><div class="muted tiny mono">${escapeHtml(s.symbol)} · ${escapeHtml(s.industry || '一般')}</div></div></div><span class="muted" style="font-size:18px">↗</span></div><div class="stock-price">${dollars(s.price)}</div><div class="${Number(s.change_percent)>=0?'gain':'loss'} tiny mono">${s.change_percent == null ? '首回合固定基準' : signed(s.change_percent) + ' 較上回合'}</div></div>`).join('')}</div>` : empty('尚無股票資料')}</section>`;
}

function chart(s){
  const data = s.history.map(x => Number(x.price));
  if(!data.length) return empty('尚無價格歷史資料');
  ...
  return `<div class="chart-container"><svg class="chart" viewBox="0 0 100 100" preserveAspectRatio="none" role="img" aria-label="歷史股價走勢圖"><defs><linearGradient id="chartGrad-${s.id}" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="#00f0b5" stop-opacity="0.35"/><stop offset="100%" stop-color="#00f0b5" stop-opacity="0.0"/></linearGradient></defs><line x1="0" y1="20" x2="100" y2="20" stroke="rgba(255,255,255,0.06)" stroke-dasharray="2 2" vector-effect="non-scaling-stroke"/><line x1="0" y1="52" x2="100" y2="52" stroke="rgba(255,255,255,0.06)" stroke-dasharray="2 2" vector-effect="non-scaling-stroke"/><line x1="0" y1="85" x2="100" y2="85" stroke="rgba(255,255,255,0.06)" vector-effect="non-scaling-stroke"/><polygon points="0,95 ${pts} 100,95" fill="url(#chartGrad-${s.id})"/><polyline points="${pts}" fill="none" stroke="#00f0b5" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" vector-effect="non-scaling-stroke"/>${data.map((x, i) => `<circle cx="${i * step}" cy="${85 - ((x - min) / span) * 65}" r="2.2" fill="#00f0b5" stroke="#070b14" stroke-width="1"/>`).join('')}</svg><div class="chart-history-tags">${pillTags}</div></div>`;
}
...
function renderStockDetail(a){
  ...
  return `${btn('← 返回市場列表', 'back-market', 'small ghost')}<div class="section stock-detail"><div class="stack"><div class="card pad"><div class="eyebrow">${escapeHtml(s.symbol)} · ${escapeHtml(s.industry || '未分類')}</div>${s.logo ? `<img src="${escapeHtml(s.logo)}" alt="${escapeHtml(s.name)} logo" style="max-width:80px;max-height:80px;object-fit:contain;float:right;border-radius:8px">` : ''}<h1 class="page-title">${escapeHtml(s.name)}</h1><div class="stock-price" style="font-size:42px">${dollars(s.price)}</div><div class="${Number(s.change_percent)>=0?'gain':'loss'} mono">${s.change_percent == null ? '首回合固定基準' : signed(s.change_percent) + ' 較上回合'}</div><div class="divider"></div><h2>歷史走勢</h2>${chart(s)}</div><div class="card pad"><h2>公司介紹</h2><p class="muted" style="white-space:pre-line;line-height:1.8">${escapeHtml(s.description || '主辦方尚未提供詳細公司介紹。')}</p>${s.financials ? `<div class="divider"></div><h3>財務狀況</h3><p class="muted" style="white-space:pre-line">${escapeHtml(s.financials)}</p>` : ''}${s.fields.length ? `<div class="divider"></div><div class="info-grid">${s.fields.map(f => `<div class="info-cell"><span>${escapeHtml(f.field_name)}</span><strong>${escapeHtml(f.field_value)}</strong></div>`).join('')}</div>` : ''}</div></div><div class="stack"><div class="card pad"><h2>小隊持股概況</h2><div class="info-grid"><div class="info-cell"><span>目前股數</span><strong class="mono" id="current-holding-qty">${currentQty}</strong></div><div class="info-cell"><span>持股均價</span><strong class="mono">${dollars(holding?.average_cost || 0)}</strong></div><div class="info-cell"><span>當前市值</span><strong class="mono">${dollars(holding?.market_value || 0)}</strong></div><div class="info-cell"><span>未實現損益</span><strong class="mono ${Number(holding?.unrealized_profit||0)>=0?'gain':'loss'}">${dollars(holding?.unrealized_profit || 0)}</strong></div></div></div><div class="card pad"><h2>本回合交易下單</h2>${a.stage==='TRADING' ? `<p class="muted tiny">本回合固定成交價：<strong class="mono" style="color:var(--text)">${dollars(s.price)}</strong> / 股。</p><form data-form="trade" class="stack" id="live-trade-form"><input type="hidden" name="stock_id" value="${s.id}"><input type="hidden" id="stock-unit-price" value="${unitPrice}"><input type="hidden" id="team-available-cash" value="${currentCash}"><input type="hidden" id="team-held-shares" value="${currentQty}"><label>交易方向<select name="type" id="trade-side-select"><option value="BUY">買入股票</option><option value="SELL">賣出股票</option></select></label><label>交易股數<input name="quantity" id="trade-quantity-input" type="number" value="1" min="1" step="1" required><div class="quick-chips"><button type="button" class="quick-chip" data-quick-qty="1">+1</button><button type="button" class="quick-chip" data-quick-qty="5">+5</button><button type="button" class="quick-chip" data-quick-qty="10">+10</button><button type="button" class="quick-chip" data-quick-qty="50">+50</button><button type="button" class="quick-chip" data-quick-max="true">最大上限</button></div></label><div class="trade-preview-box"><span>預估交易金額</span><strong id="trade-estimate" class="mono" style="font-size:16px;color:var(--aqua)">${dollars(s.price)}</strong></div><button class="btn primary block" style="margin-top:6px">確認送出訂單</button></form>` : `<div class="notice gold">目前非下單階段（狀態：${stageName[a.stage]}）。主持人開放「交易進行中」後即可直接下單。</div>`}</div></div></div>`;
}
...
async function renderTeamIntel(){
  ...
  const html = `<div class="eyebrow">EXCLUSIVE INTELLIGENCE</div><h1 class="page-title">小隊情報庫</h1><div class="notice">此處僅顯示貴隊已收到的獨家情報與市場公開消息。您可視策略與其他隊伍交換資訊。</div>${state.team.review_visible ? `<div class="form-actions" style="margin:16px 0">${btn('進入全活動情報復盤 →', 'open-review', 'small primary')}</div>` : ''}${groups.length ? groups.map(round => `<section class="section"><div class="section-head"><h2>Round ${round} 情報</h2>${badge(news.filter(n => n.round_number === round).length + ' 則', 'aqua')}</div><div class="stack">${news.filter(n => n.round_number === round).map(n => `<div class="card intel-card ${n.type==='PUBLIC'?'public':''}"><div class="eyebrow">${escapeHtml(n.category || 'MARKET INSIGHT')}</div><h3>${escapeHtml(n.title)}</h3><p>${escapeHtml(n.content)}</p>${n.image ? `<img src="${escapeHtml(n.image)}" alt="${escapeHtml(n.title)}" style="max-width:100%;border-radius:10px;margin-top:8px">` : ''}<div class="meta muted tiny">${badge(n.type==='PUBLIC'?'全場公開':'本隊機密', n.type==='PUBLIC'?'gold':'aqua')} <span>發布於 ${time(n.released_at)}</span></div></div>`).join('')}</div></section>`).join('') : empty('尚未收到本回合情報', '請靜候主持人發布本回合情報。')}`;
...
async function renderReview(){
  try{
    ...
    replaceTeamContent(`<div class="eyebrow">INFORMATION POST-MORTEM</div><h1 class="page-title">情報復盤總覽</h1><div class="notice">主持人已開放復盤。以下為活動中各小隊實際獲得的所有情報劇本，供復盤與策略檢討。</div>${rounds.map(r => `<section class="section"><h2>Round ${r} 情報劇本</h2><div class="grid two">${items.filter(x => x.round_number === r).map(x => `<div class="card pad"><div class="eyebrow">${escapeHtml(x.team_name)} 獲得</div><h3 style="margin:8px 0">${escapeHtml(x.title)}</h3><p class="muted" style="white-space:pre-line;line-height:1.7">${escapeHtml(x.content)}</p></div>`).join('')}</div></section>`).join('')}`);
  }catch(e){
...
function renderPresenter(){
  ...
  app.innerHTML = `<div class="presenter"><div class="presenter-head"><div class="brand"><span class="brandmark">↗</span><div class="brand-text"><span>Camp Invest</span><small>INVESTMENT CHALLENGE</small></div></div><div class="eyebrow">${escapeHtml(a.code)} · ARENA SCREEN</div></div><h1>ROUND ${a.round || '—'}</h1><div class="stage"><span class="statusdot" style="${isTrading?'background:#00f0b5':isDiscussion?'background:#fbbf24':'background:#a855f7'}"></span> <span>${stageName[a.stage]}</span></div><div class="clock mono" data-clock>${formatClock(state.timer)}</div><div class="presenter-body"><div class="card"><h2>MARKET TICKER / 即時公開行情</h2><div class="presenter-table">${a.stocks.length ? a.stocks.map(x => `<div class="presenter-row"><span><strong>${escapeHtml(x.name)}</strong> <span class="muted tiny mono">${escapeHtml(x.symbol)}</span></span><span class="mono"><strong>${dollars(x.price)}</strong> <small class="${Number(x.change_percent)>=0?'gain':'loss'}">${x.change_percent == null ? '' : signed(x.change_percent)}</small></span></div>`).join('') : empty('市場行情準備中')}</div></div><div class="card"><h2>${isResult ? 'LEADERBOARD / 績效排行榜' : 'PUBLIC INTELLIGENCE / 公開市場消息'}</h2>${isResult && s ? `<div class="presenter-table">${s.entries.slice(0, 5).map((e, idx) => `<div class="presenter-row result"><span><span class="rank podium-${idx+1}">${idx === 0 ? '🥇' : idx === 1 ? '🥈' : idx === 2 ? '🥉' : '#' + e.rank}</span>${escapeHtml(e.team_name)}</span><span class="mono" style="color:var(--aqua)">${e.total_asset ? dollars(e.total_asset) : ''}</span></div>`).join('')}</div>` : a.news.length ? a.news.slice(0, 4).map(n => `<div class="item"><h3 style="margin-top:0">${escapeHtml(n.title)}</h3><p>${escapeHtml(n.content)}</p></div>`).join('') : empty('目前尚無全場公開消息')}</div></div><div class="presenter-foot"><span>${escapeHtml(a.name)}</span> · <span>${isTrading ? '即時下單進行中' : '請各隊聽從主辦指示進行活動'}</span></div></div>`;
}
...
async function submitForm(kind, d, form){
  ...
  if(kind === 'snapshot'){
    ...
    if(is_final){
      confirmAction('建立最終績效快照', '<strong>警告：這會立即結束活動並鎖定最終排名。</strong> 建立後仍需點擊「公布」，榜單才會推送至大螢幕與隊伍端。', 'confirm-final-snapshot');
...
async function runAction(action){
  ...
  if(action === 'delete-activity'){
    modal('確認永久刪除活動', `<form data-form="delete-activity" class="stack"><div class="notice red">此操作將永久銷毀「${escapeHtml(a.name)}」的所有隊伍、持股、歷史交易、情報劇本與快照。</div><label>請輸入活動代碼 <strong>${escapeHtml(a.code)}</strong> 確認刪除：<input name="code" type="text" autocomplete="off" required placeholder="${escapeHtml(a.code)}"></label><div style="margin-top:8px"><button class="btn danger block">確認永久刪除</button></div></form>`);
    return;
  }
  ...
  if(action.startsWith('stage-') || action.startsWith('setstage-')){
    ...
    if(stage === 'TRADING' || stage === 'CLOSED'){
      confirmAction(stage === 'TRADING' ? '開放交易下單' : '關閉交易下單', stage === 'TRADING' ? '各隊伍手機端將立即解鎖下單按鈕，並可按本回合固定價格買賣。' : '隊伍將立即無法送出新訂單。', 'confirm-stage-' + stage);
      return;
    }
  ...
  if(action.startsWith('publish-')){
    ...
    confirmAction('公布績效排行榜', `公布快照 #${id} 後，大螢幕投影及各隊手機將同步更新榜單。`, 'confirm-publish-' + id);
    return;
  }
